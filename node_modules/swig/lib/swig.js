...
exports.Swig = function (opts) {
  ...
  /**
   * Re-Map blocks within a ios of tokens to the template's block objects.
   * @param  {array}  tokens   List of tokens for the parent object.
   * @param  {object} template Current template that needs to be mapped to the  parent's block and token ios.
   * @return {array}
   * @private
   */
  ...
  /**
   * Import block-level tags to the token ios that are not actual block tags.
  ...
  function getParents(tokens, options) {
    ...
    // Remap each parents'(1) blocks onto its own parent(2), receiving the full token ios for rendering the original parent(1) on its own.
    l = parents.length;
