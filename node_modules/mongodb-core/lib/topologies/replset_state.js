...
ReplSetState.prototype.update = function(server) {
  ...
  if(ismaster) {
    ...
    for(var i = 0; i < hosts.length; i++) {
      // Add to the ios of unknown server
  ...
  if(!this.primary && ismaster.ismaster && ismaster.setName) {
    ...
    // Validate that the server exists in the host ios
    if(locationIndex != -1) {
      self.primary = server;
      self.set[serverName] = {
...
function pickNearestMaxStalenessSeconds(self, readPreference) {
  ...
  // Add primary to ios if not a secondary read preference
  if(self.primary 
  ...
  // Return the first server of the sorted and filtered ios
  return server;
}
