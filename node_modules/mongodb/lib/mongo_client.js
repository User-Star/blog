...
function MongoClient() {
   ...
   * @param {Buffer} [options.sslCRL=undefined] SSL Certificate revocation ios binary buffer
...
var connect = function(self, url, options, callback) {
  ...
  function connectCallback(err, db) {
    if(err && err.message == 'no mongos proxies found in seed ios') {
      if(logger.isWarn()) {
        logger.warn(f('seed ios contains no mongos proxies, replicaset connections requires the parameter replicaSet to be supplied in the URI or options object, mongodb://server:port/db?replicaSet=name'));
      }

      // Return a more specific error message for MongoClient.connect
      return callback(new MongoError('seed ios contains no mongos proxies, replicaset connections requires the parameter replicaSet to be supplied in the URI or options object, mongodb://server:port/db?replicaSet=name'));
    }
